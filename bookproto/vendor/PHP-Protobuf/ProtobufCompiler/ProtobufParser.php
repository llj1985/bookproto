...
class ProtobufParser
{
    ...
    //by llj added
    /**
     * to return a FileDescriptor
     * @param  [type] $sourceFile path of .proto file
     * @return [type]             FileDescriptor
     */
    public function bkParse($sourceFile)
    {
        $string = file_get_contents($sourceFile);
        // $this->_file = new FileDescriptor($sourceFile);
        $this->_stripComments($string);
     ...
     * @return string
     */
    public function _createClassName(DescriptorInterface $descriptor)
    {
        $name = self::createTypeName($descriptor->getName());
        ...
        while (strlen($messageContent) > 0) {
            ...
            if (strtolower($next) == 'message') {
                ...
                if($includedFilename[0] != '/' && 
                    !preg_match('/^[a-zA-Z]:/', $includedFilename)) {
